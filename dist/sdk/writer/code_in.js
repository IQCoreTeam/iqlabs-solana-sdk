...
function toChunks(data) {
    if (Array.isArray(data))
        return data;
    if (Buffer.byteLength(data, "utf8") <= constants_1.CHUNK_SIZE)
        return [data];
    const chunks = [];
    let chunk = "";
    let chunkBytes = 0;
    for (const char of data) {
        ...
        if (chunkBytes + charBytes > constants_1.CHUNK_SIZE) {
            chunks.push(chunk);
            chunk = char;
...
async function prepareCodeIn(input, data, filename, method = 0, filetype = "", onProgress, speed) {
    ...
    const useInline = inlineMetadata.length > 0 &&
        Buffer.byteLength(inlineMetadata, "utf8") <= constants_1.DIRECT_METADATA_MAX_BYTES;
    const metadata = useInline ? inlineMetadata : JSON.stringify(baseMetadata);
    // Upload chunks (linked-list vs session)
    let onChainPath = "";
    const useSession = !useInline && totalChunks >= constants_1.DEFAULT_LINKED_LIST_THRESHOLD;
    let sessionAccount;
    let sessionFinalize = null;
    if (!useInline) { // useInline =  data + metadata < 900 bytes
        if (!useSession) { //useSession = data>8500 bytes
            onChainPath = await (0, uploading_methods_1.uploadLinkedList)(connection, signer, builder, user, codeAccount, chunks, method, onProgress, { speed });
        }
        else {
            onChainPath = await (0, uploading_methods_1.uploadSession)(connection, signer, builder, programId, user, userState, seq, chunks, method, { onProgress, speed });
    ...
    const feeReceiver = new web3_js_1.PublicKey(constants_1.DEFAULT_WRITE_FEE_RECEIVER);
    const isDirectPath = !useSession && onChainPath.length === 0;
    const iqAta = isDirectPath
        ? await (0, ata_1.resolveAssociatedTokenAccount)(connection, user, new web3_js_1.PublicKey(constants_1.DEFAULT_IQ_MINT), false)
        : null;
    return {
        builder,
        user,
...
async function codeIn(input, data, filename, method = 0, filetype = "", onProgress, speed) {
    const { builder, user, userInventory, onChainPath, metadata, sessionAccount, sessionFinalize, feeReceiver, iqAta, } = await prepareCodeIn(input, data, filename, method, filetype, onProgress, speed);
