...
exports.resolveConnectionStatus = resolveConnectionStatus;
...
function decodeTableMeta(data) {
    ...
    return {
        columns: decoded.column_names.map((value) => Buffer.from(value).toString("utf8").replace(/\0+$/, "")),
        ...
        gate: {
            ...
            gateType: decoded.gate.gate_type,
        },
...
function decodeConnectionMeta(data) {
    ...
    return {
        ...
        columns: decoded.column_names.map((value) => Buffer.from(value).toString("utf8").replace(/\0+$/, "")),
        idCol: Buffer.from(decoded.id_col).toString("utf8").replace(/\0+$/, ""),
        extKeys: decoded.ext_keys.map((value) => Buffer.from(value).toString("utf8").replace(/\0+$/, "")),
        name: Buffer.from(decoded.name).toString("utf8").replace(/\0+$/, ""),
        gate: decoded.gate,
...
function resolveConnectionStatus(status) {
